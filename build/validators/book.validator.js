...
const idValidator = express_validator_1.body('_id').notEmpty().withMessage('Must contains an _id');
exports.idValidator = idValidator;
const titleValidator = express_validator_1.body('title').isString().withMessage('Must be a string')
    .isLength({ min: 3, max: 15 }).withMessage('Must be between 3 and 15 letter').trim();
exports.titleValidator = titleValidator;
const priceValidator = express_validator_1.body('price').isFloat({ min: 0.00 }).withMessage('Must be a float with minimum value has 0.00').trim();
exports.priceValidator = priceValidator;
const descriptionValidator = express_validator_1.body('description').notEmpty().withMessage('Must not be empty')
...
const authorValidator = express_validator_1.body('author').notEmpty().withMessage('Must not be empty').isString().withMessage('Must be a string')
